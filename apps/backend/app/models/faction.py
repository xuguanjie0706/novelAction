...
class Faction(Base):
    ...
    parent_faction_id = Column(UUID(as_uuid=True), ForeignKey("factions.id"), nullable=True)
    ...
    # 领导层
    leader_character_id = Column(UUID(as_uuid=True), ForeignKey("characters.id"), nullable=True)
    key_members = Column(JSON, default=list)
    # [{character_id: "...", title: "长老", note: "执法堂首席长老"}]
    ...
    # 对主角的关系态度（随剧情变化）
    attitude_to_protagonist = Column(String(20), default="neutral")
    # friendly/hostile/neutral/subordinate/superior
