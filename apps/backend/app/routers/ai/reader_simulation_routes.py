"""
reader_simulation_routes.py — 追读模拟 · 章末钩子检测 · 故事线悬空检测 · 章节综合分析（薄壳）

资源边界：
  仅包含 FastAPI endpoint handler。业务逻辑已拆分到：
    reader_simulation_schemas.py  — Pydantic 数据结构
    reader_simulation_helpers.py  — 辅助函数（上下文构建 / 统计 / 单次 LLM 分析）
...
from app.routers.ai.reader_simulation_helpers import (
    _build_stats_from_records,
    _derive_drop_risk,
    _extract_hook_tail,
    _get_chapter_summary,
    _single_shot_analysis,
)
...
async def chapter_analysis(
    ...
    # ── AI 单次调用 ──────────────────────────────────────────────────────────
    result = await _single_shot_analysis(
