...
⓪ 你必须以【情节锁定表】与【伏笔日程锁定表】为准：
   ...
   - 伏笔日程表：程序已注入「禁止提前完整埋设」与「本章应埋/加热」清单；**不要在 risks 中逐条复述**
     远期核心谜题日程（那是背景约束，不是本章待修问题）。must_events 不得违背日程表；
...
⑤ 风险扫描（risks）
   只列需要改章纲/改正文/改设定的**具体可行动矛盾**（通常 0～3 条），逐条对照资料：
   - continuity：连续性矛盾（人物状态/位置/事件顺序）
   - foreshadow：伏笔违约（漏收/误收/章纲与日程**直接冲突**）
   - setting：设定违规（境界/势力/道具与世界观矛盾）
   - ooc：人物OOC（行为与性格/价值观/创伤明显冲突）
   - pacing：节奏预警（与前章钩子期望落差）
   **禁止**把「第N章才埋的核心谜题」逐条写入 risks（已在伏笔日程表展示）。
