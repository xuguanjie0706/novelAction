...
def build_dabai_draft_prompt(
    ...
    *,
    ...
    system = (
        ...
        "3. 情绪反转须有转折拍铺垫；见证者反应分级递进，但每章换一套写法"
        "（愣→疑→惊→服／嗤笑→脸僵→冷汗→噤声／起哄→骚动→倒抽冷气→噤声 等择一），"
        "落到具体脸色动作，禁止每章同一句套话；\n"
        f"4. 篇幅：正文 {lo}～{hi} 字，严禁超过 {hi} 字；五拍各用紧凑篇幅，禁止重复铺陈；\n"
        "5. 衔接硬约束：开头必须正面承接【上章结尾】与上章末钩子；人物境界/位置/敌对关系"
        "以【当前状态】块为准；位置变化必须在正文交代移动过程，禁止无交代瞬移；"
        "禁止与【前情提要】【相关记忆】矛盾；\n"
        "6. ★视角硬约束（限知第三人称，全程贴住主角）★：只写主角此刻能看到/听到/"
        "感觉到/推断到的；禁止上帝视角旁白——不写主角不可能知道的他人内心、幕后真相、"
        "未来结果、设定来历全貌，禁止作者下场分析点评（『其实这是…』『殊不知…』『这正是…的奥妙』）。"
        "出现主角未知的新事物（新道具/金手指/能力/境界/陌生人）时，主角第一反应须是懵/疑/不解，"
        "先困惑或追问，再由懂行的人解释或自己摸索验证才弄懂，严禁一上来就准确报出其名称/来历/用法/数值；\n"
