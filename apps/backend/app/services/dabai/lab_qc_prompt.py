...
_LAB_QC_SYSTEM = (
    ...
    "【叙事视角验收（限知第三人称 · 硬伤=阻断）】正文须全程贴住主角："
    "出现上帝视角旁白（写主角不可能知道的他人内心/幕后真相/未来结果/设定来历全貌）、"
    "或作者下场分析点评（『其实这是…』『殊不知…』『这正是…的奥妙』）、"
    "或新事物登场时主角不懵、无人解释也无自我疑问、却被旁白直接报出名称来历用法数值、"
    "或其他角色凭空知道未公开的真相/主角底牌却无交代来源——"
    "★一律写入 critical_violations（kind=pov），禁止只写 chapter_suggestions★。\n"
    "【配角境界验收（硬伤=阻断）】出场配角境界须与【出场人物境界锁定】/人物档案一致；"
    "旁白直接报错层、或与章纲五拍隐含同境对决矛盾——"
    "★写入 critical_violations（kind=realm），continuity_score≤50★。\n"
    "【战力可信度验收（硬伤=阻断）】碾压/秒杀/废人若无当场依据"
    "（金手指明示动用、阴寒腐蚀特性、对方破绽或轻敌、奇袭过程）——"
    "★写入 critical_violations（kind=credibility），beats.payoff 判 miss★。\n"
    "chapter_suggestions 仅写非阻断微调；[境界]/[可信]/[视角] 前缀仅兼容，优先 critical_violations。\n"
    ...
    "【自然度验收】检查套话/模板化见证者反应："
    "「不敢置信/如坠冰窟/眼珠子都要/倒吸凉气/阴寒灵力轰然倒灌/微微颤动」等若本章或近章反复出现，"
    "naturalness_score 应≤70，overused_phrases 列出具体短语；"
    "witness_reaction_issue 写见证者是否人人同一模板、是否缺物件/动作转义。\n"
    "【可信度验收】对主角有利的转机/巧合/碾压若无前文铺垫、也无当场交代的依据"
    "（来历/动机/代价/对方破绽或轻敌），属「开挂硬翻/天降好运」——"
    "★必须写入 critical_violations（kind=credibility），禁止仅写 [可信] 建议★。"
)
...
def build_lab_qc_prompt(
    ...
    *,
    ...
    if phrase_guard_block.strip():
        parts.append(phrase_guard_block.strip())
    ...
    parts.append(
        ...
        '  "overused_phrases": ["套话1", "套话2"],\n'
        '  "witness_reaction_issue": "无问题留空",\n'
        '  "critical_violations": [{"kind": "realm|pov|credibility", "message": "硬伤说明"}],\n'
        '  // 有任一硬伤必须填满 critical_violations；无硬伤则 []\n'
        '  "chapter_suggestions": ["≤3条非阻断微调，禁止文采类"],\n'
