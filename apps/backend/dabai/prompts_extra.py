...
def title_blurb(ctx: dict, cfg: DabaiConfig) -> tuple[str, str]:
    ...
    user = (
        ...
        + benchmark_block(ctx)
        + _volumes_digest(ctx) + "\n"
        "为本书做【书名海选 + 上架简介】，不省 token、把弹药打满。\n"
        "书名 16-20 个候选，6 种策略轮换且每种至少 2 个："
        "金手指直给（开局签到百年）/反差装逼（废柴的我吞了天骄）/境界宣言（我以凡躯镇万仙）/"
        "台词式（就凭你也配退婚？）/悬念式（谁在我脑子里收尸）/身份反转（杂役弟子竟是…）。"
        "每个按番茄移动端点击直觉打分（0-100），并标该名最能勾住的是哪类读者。\n"
        "简介给两版：blurb（80-150字，3-6短句，结构=憋屈处境+金手指登场+打脸承诺+一句钩子）"
        "+ blurb_short（30-50字超短钩，首页信息流用）；口语直给，禁文艺腔、禁世界观说明。\n"
        "返回 JSON：\n"
        "{\n"
        '  "title_candidates": [\n'
        '    {"title": "≤12字", "strategy": "策略名", "score": 90,\n'
        '     "hook_for": "最能勾住的读者类型", "reason": "≤20字 为何能点"}\n'
        "  ],\n"
        '  "chosen_title": "得分最高且不与对标书撞名的一个",\n'
        '  "runner_up": "次选书名（备用）",\n'
        '  "tagline": "一句话标语/副标题（≤16字，可印封面）",\n'
        '  "cover_concept": "封面画面一句话（主角+金手指+气势，给美工的 brief）",\n'
        '  "blurb": "上架简介（80-150字）",\n'
        '  "blurb_short": "超短钩（30-50字）"\n'
        "}\n"
        "硬规则：chosen_title 禁止与对标书名雷同（换皮可以，撞名不行）；"
        "候选 16 个起步、6 种策略全覆盖；至少 4 个带数字或具体名词（人/宝/境界），"
        "禁止全是抽象四字；chosen_title 与 runner_up 不能是同一种策略。"
    )
    return SYS_BASE, user
