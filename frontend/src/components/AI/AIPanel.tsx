...
export default function AIPanel({ projectId }: Props) {
  ...
  const [modelProfile, setModelProfile] = useState<'local' | 'gemini'>('local')
  ...
  const extractMemory = async () => {
    ...
    setLoading(true)
    try {
      const res = await aiApi.extractMemory(projectId, activeChapterId, modelProfile)
  ...
  return (
    <aside className="w-80 flex flex-col border-l border-gray-100 bg-white shrink-0">
      ...
      <div className="px-4 py-3 border-b border-gray-100">
        ...
        <select
          value={modelProfile}
          onChange={(e) => setModelProfile(e.target.value as 'local' | 'gemini')}
          className="w-full text-sm border border-gray-200 rounded-lg px-2.5 py-2 focus:outline-none focus:ring-1 focus:ring-amber-400"
        >
