...
function FullGenConfigModal({
  ...
  const handleStart = () => {
    ...
    onDispatch({ scale_hint: scaleHint, model_profile: modelProfile, clear_existing: clearExisting })
    onClose()
  }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div>
            ...
            >
              <option value="default">本地（qwen3）— 批量分段生成</option>
              <option value="gemini">Gemini — 更快更准</option>
            </select>
          </div>
...
function BatchExpandConfigModal({
  ...
  targetCount: number
  onClose: () => void
  onDispatch: (chapterCount: number, modelProfile: 'default' | 'gemini') => void
}) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 overflow-hidden">
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-gray-100 bg-gray-50">
          ...
          <button
            onClick={() => { onDispatch(chapterCount, modelProfile); onClose() }}
            className="flex items-center gap-2 px-4 py-1.5 text-sm font-medium bg-amber-500 hover:bg-amber-600 text-white rounded-lg shadow-sm"
          >
            <Sparkles size={14} />
            加入队列
...
export default function OutlinePage() {
  ...
  // ── 批量展开 → 派发到队列 ────────────────────────────────
  const handleDispatchBatchExpand = (chapterCount: number, modelProfile: 'default' | 'gemini') => {
    if (!projectId) return
